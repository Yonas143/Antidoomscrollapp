...
extension PumpThenSettle on WidgetTester {
  Future<void> pumpThenSettle({
    ...
    await pumpAndSettle(duration, phase, timeout ?? const Duration(seconds: 10));
  }
}
...
void main() {
  ...
  group('Authentication Flow Goal Tests', () {
    ...
    testWidgets('Goal: User can log in successfully and see the dashboard', (WidgetTester tester) async {
      // ARRANGE
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          // ],
          child: app.MindSnapApp(), // Assuming MyApp is your root widget from main.dart
        ),
      );
      ...
      expect(find.byType(LoginPage), findsOneWidget, reason: 'Should start on or navigate to LoginPage');
      print('Test: Verified on LoginPage.');

      // ACT
      // Enter credentials
      await tester.enterText(find.byKey(const Key('login_username_field')), 'test@example.com'); // Use your test credentials
      await tester.enterText(find.byKey(const Key('login_password_field')), 'password123'); // Use your test credentials
      ...
      await tester.pumpThenSettle(timeout: const Duration(seconds: 10)); // Adjust timeout as needed

      // ASSERT
      // Verify navigation to dashboard and login page is gone
      expect(find.byType(LoginPage), findsNothing, reason: 'LoginPage should be gone');
      expect(find.byType(UserDashboard), findsOneWidget, reason: 'Should be on UserDashboardPage');
    ...
    testWidgets('Goal: User sees error on invalid login', (WidgetTester tester) async {
      // ARRANGE:
      ...
      // ACT
      await tester.enterText(find.byKey(const Key('login_username_field')), 'wrong@example.com');
      ...
      // ASSERT
      expect(find.byType(UserDashboard), findsNothing, reason: 'Should NOT navigate on failed login');
      expect(find.byType(LoginPage), findsOneWidget, reason: 'Should remain on LoginPage');
      ...
      print('Test Goal: "User sees error on invalid login" - PASSED (assuming error message appears)');
    });
